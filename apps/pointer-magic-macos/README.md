# Pointer Magic native bedrock

This package is the local macOS foundation beside the browser experiments in
`../showcase`. macOS continues to own and draw the real cursor. Pointer Magic
...
The shelf is provider-driven. Hosts assemble a `PointerContextPacket` (AX + settled
OCR + scene probe); registered `ShelfProviding` implementations return a declarative
`ShelfDocument`. The renderer paints either a compact agent pill (provider mark,
directory, status) or an expanded Gemini-style card (context chips, prompt, action
pills). See [`docs/SHELF_PROVIDERS.md`](docs/SHELF_PROVIDERS.md).

Left-clicking ✦ parks the shelf at the cursor and makes it clickable; right-clicking
✦ opens the short settings menu. Compact shelves activate the agent on click; expanded
shelves activate through action pills. × hides that exact update, and an eight-second
hard lease restores the click-through follower. Parking needs no global keyboard
shortcut. Right Option and Right Command are not used for shelf interaction.
...
cursor replacement, cursor warp, CGEvent posting, keystroke injection, or action from
hover in this layer. The one intentional side effect is **Apple Events focus** after an
explicit parked-shelf click (Ghostty / Terminal / Cursor). Pointer Magic never steals
focus in the background and never types into terminals.

Shelf updates use two lanes:

- **Present (hard)** — pointer pose follow plus an immediate compact agent document.
  `acceptPointerSample` never awaits OCR, propose, actors, or network.
- **Enrichment (soft)** — AX settle / agent snapshot kicks OCR + sample providers + merge
  into a generation-fenced latest-wins mailbox. Late or superseded results drop; the
  shelf keeps the last good document.

While Pointer Magic is on, a separate local perception lane analyzes a bounded
...
kinds, bounds, provenance labels, and evidence-source names. Turning Pointer Magic
off stops event capture, semantic lookup, local screen analysis, and every overlay.
The capped local feedback log is `~/Library/Application Support/Pointer Magic/perception-feedback.jsonl`.
...
CGEventTap thread
  ...
  -> fixed 128-event SPSC ring (buttons + discrete modifier edges stay ordered)
...
swift test
swift run -c release PointerMagicBench
./scripts/build-app.sh --open
```

The app is assembled at `.build/app/Pointer Magic.app` and ad-hoc signed by
default. The build embeds the Apple Events entitlement used only by the explicit
shelf-click terminal focus path. For a stable TCC identity across rebuilds, set a
real local signing identity:

```bash
POINTER_MAGIC_CODESIGN_IDENTITY="Apple Development: …" \
  ./scripts/build-app.sh --open
```
...
Input Monitoring covers the low-level pointer event tap used for shake activation;
typed text and ordinary keys are not collected. Agent-shelf parking is an ordinary
AppKit menu-bar-button action. Screen Recording supplies the bounded local crop used for
...
still supports the ✦ parking button for the agent shelf and exposes a manual menu fallback
for the separate Perception Lens. Without Accessibility, visual analysis stays disabled rather than
...
`PointerMagicBench` guards the atomic mailbox against transport regressions. The
