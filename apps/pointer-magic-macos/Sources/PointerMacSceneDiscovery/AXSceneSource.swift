...
final class AXSceneScanWorker: @unchecked Sendable {
    ...
    func start() {
        ...
        thread.name = "PointerMagic.AXDiscoveryScan"
...
final class AXLifecycleTimerRegistration: @unchecked Sendable {
    ...
    static func start(
        ...
        let heartbeatTimer = DispatchSource.makeTimerSource(
            flags: [],
            queue: DispatchQueue(label: "pointer-magic.ax-discovery.heartbeat", qos: .utility)
        )
        ...
        let periodicRescanTimer = DispatchSource.makeTimerSource(
            flags: [],
            queue: DispatchQueue(
                label: "pointer-magic.ax-discovery.periodic-rescan",
                qos: .utility
            )
        )
