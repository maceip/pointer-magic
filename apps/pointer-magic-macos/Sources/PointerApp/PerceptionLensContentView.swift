...
final class PerceptionLensContentView: NSView {
    ...
    private func render() {
        ...
        if let bounds = candidate.bounds,
           ...
           bounds.size.width < Double(Int.max),
           bounds.size.height < Double(Int.max)
        {
            boundsLabel.stringValue =
                "Context crop · object \(Int(bounds.size.width))×\(Int(bounds.size.height))"
        } else {
