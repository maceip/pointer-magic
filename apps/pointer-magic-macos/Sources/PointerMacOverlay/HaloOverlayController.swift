...
public final class HaloOverlayController: NSObject {
    ...
    /// Publishes a sampled physical modifier edge through the same serial MainActor
    /// lane as position-only pointer frames. This preserves one monotonically ordered
    /// sequence even when the privileged event tap is unavailable.
    public func acceptPositionOnlyInteractionModifier(
        _ transition: PointerInteractionModifierTransition
    ) {
        guard isRunning, captureMode == .positionOnly else { return }
        let nowNs = DispatchTime.now().uptimeNanoseconds
        _ = acceptFallbackPosition(nowNs: nowNs)
        guard isRunning, captureMode == .positionOnly else { return }
                ...
                discreteEvents: [
                    PointerDiscreteEvent(
                        ...
                        modifiers: PointerModifierFlags(
                            rawValue: CGEventSource.flagsState(.combinedSessionState).rawValue
                        )
                    ),
                ]
            )
