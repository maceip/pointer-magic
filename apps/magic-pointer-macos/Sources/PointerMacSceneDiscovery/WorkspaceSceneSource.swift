...
final class WorkspaceCheckpointLane: @unchecked Sendable {
    static let threadName = "MagicPointer.WorkspaceCheckpoint"
...
final class WorkspaceSceneWorker: @unchecked Sendable {
    ...
    func start() {
        ...
        let thread = Thread { [weak self] in self?.threadMain() }
        thread.name = "MagicPointer.WorkspaceDiscovery"
        thread.qualityOfService = .utility
        self.thread = thread
        thread.start()
        condition.unlock()
...
final class WorkspaceLifecycleTimerRegistration: @unchecked Sendable {
    ...
    static func start(
        ...
        let heartbeatTimer = DispatchSource.makeTimerSource(
            flags: [],
            queue: DispatchQueue(
                label: "magic-pointer.scene-discovery.heartbeat",
                qos: .utility
            )
        )
        ...
        let periodicCensusTimer = DispatchSource.makeTimerSource(
            flags: [],
            queue: DispatchQueue(
                label: "magic-pointer.scene-discovery.periodic-census",
                qos: .utility
            )
        )
