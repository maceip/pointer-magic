...
/// Demo feature layer for the explicit shake -> follow -> Right Option clutch.
///
/// The bedrock remains passive. This controller owns the short-lived gesture state,
/// its ordered input observation, and the exact-size interactive panel.
@MainActor
final class MagicPointerInteractionController {
    ...
    private var shakeDetector = ShakeActivationDetector()
    ...
    var primaryActionTitle: String {
        clutchState == .idle ? "Show Magic Pointer here" : "Hide Magic Pointer"
    }
    ...
    var statusText: String {
        ...
        case .idle:
            supportsGlobalInput
                ? "Idle — shake to activate"
                : "Idle — activate from this menu"
    ...
    var instructionText: String {
        supportsGlobalInput
            ? "Shake, then hold the right Option key"
            : "Input Monitoring enables shake and Right Option"
    }
    ...
    private func accept(_ frame: PointerFrame) {
        ...
        if shakeDetector.observe(frame) {
            perception.cancelLiveSample()
            beginFollowing(from: bedrock.currentFrame() ?? frame)
        } else {
    ...
    private func accept(_ event: PointerDiscreteEvent) {
        ...
        case .buttonDown, .buttonUp, .scroll:
            // A display-cadence frame can coalesce away a short click or drag. The
            // ordered lane therefore also disarms any candidate shake.
            shakeDetector.reset()
        }
    }
    ...
    private func presentFollower(using frame: PointerFrame, lease: PointerClutchLease) {
        ...
        let scene = OverlayScene(
            sourceID: "magic-pointer.right-option-clutch",
            generation: frame.generation,
            createdAtNs: nowNs,
