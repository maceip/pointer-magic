...
test("server-renders the Pointer Magic research lab", async () => {
  const html = await renderedPage("index");
  assert.match(html, /<title>Pointer Magic Lab — Five Working Experiments<\/title>/i);
...
test("ships interactive concepts and removes starter infrastructure", async () => {
  const [component, page, layout, packageJson] = await Promise.all([
    readFile(new URL("app/pointer-magic-lab.tsx", root), "utf8"),
  ...
  assert.match(page, /<PointerMagicLab \/>/);
  assert.match(layout, /Pointer Magic Lab — Five Working Experiments/);
